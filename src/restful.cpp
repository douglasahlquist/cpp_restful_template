/*
 * @Filename: restful.cpp
 * @Author: Douglas Ahlquist <douglas.ahlquist@gmail.com>
 * @Description: wraps around libcurl to make HTTP requests
 */
...
Handle::response Handle::exec() {
        ...
        if (curl) {
                ...
                switch (method) {
                        /* Implements only the basic methods */
...
Handle::response Handle::execPost() {
        ...
        if (!post_content.empty()) {
                ...
                res.body =
                    "Empty post content not allowed.";
                res.code = -1;
                return res;
        }
        // Write callback function
